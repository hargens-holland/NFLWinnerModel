...
class NFLPredictionModel:
    ...
    def predict_game(self, game_data):
        try:
            if not game_data['stadium_neutral']:
                game_data['home_field_advantage'] = 1.0
                game_data['home_team_win_pct'] *= self.HOME_ADVANTAGE_WEIGHT
            else:
                game_data['home_field_advantage'] = 0.0

            game_df = pd.DataFrame([game_data])
            X_game = self.preprocessor.transform(game_df)
            raw_prediction = self.model.predict(X_game, verbose=0)[0][0]

            # Adjust prediction threshold for home advantage
            home_threshold = 0.45
            prediction = np.clip(raw_prediction, 0.001, 0.999)
            result = "Home Team Wins" if prediction >= home_threshold else "Away Team Wins"
